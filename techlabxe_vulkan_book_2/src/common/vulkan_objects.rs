...
impl ShaderReadOnlyImageObject {
    ...
    /// ダミーの1x1画像を用意する。
    pub fn create_dummy_image(
        allocator: Rc<vk_mem::Allocator>,
        device: Rc<Device>,
        graphics_command_pool: &vk::CommandPool,
        graphics_queue: &vk::Queue,
    ) -> Result<Self> {
        let (image_width, image_height) = (1, 1);
...
/// デバイスローカルなイメージのオブジェクト。
/// LayoutはUNDEFINED。
/// UsageはColorAttachmentとSampled、
/// vk_memを利用してアロケートする。
/// Dropで自動的にメモリを開放する。
pub struct TextureRenderingImageObject {
    image: vk::Image,
    allocation: vk_mem::Allocation,
    view: vk::ImageView,
    allocator: Rc<vk_mem::Allocator>,
...
impl TextureRenderingImageObject {
    /// 新しく生成する。
    pub fn new(
        device: Rc<Device>,
        allocator: Rc<vk_mem::Allocator>,
...
impl Drop for TextureRenderingImageObject {
    fn drop(&mut self) {
        self.allocator
            .destroy_image(self.image, &self.allocation)
            .expect("Failed to destroy image");
...
/// デバイスローカルなイメージのオブジェクト。
/// LayoutはUNDEFINED。
/// UsageはDepthStencilAttachmentとSampled、
/// vk_memを利用してアロケートする。
/// Dropで自動的にメモリを開放する。
pub struct TextureRenderingDepthObject {
    image: vk::Image,
    allocation: vk_mem::Allocation,
    view: vk::ImageView,
    allocator: Rc<vk_mem::Allocator>,
...
impl TextureRenderingDepthObject {
    /// 新しく生成する。
    pub fn new(
        device: Rc<Device>,
        allocator: Rc<vk_mem::Allocator>,
...
impl Drop for TextureRenderingDepthObject {
    fn drop(&mut self) {
        self.allocator
            .destroy_image(self.image, &self.allocation)
            .expect("Failed to destroy image");
...
impl RenderPassObject {
    ...
    /// RenderPassObjectの作成。
    /// DepthOnly。
    pub fn new_depth_only(device: Rc<Device>) -> Result<Self> {
        // Attachments
        let attachments = [vk::AttachmentDescription::builder()
            .format(vk::Format::D32_SFLOAT)
            .samples(vk::SampleCountFlags::TYPE_1)
...
impl FramebufferObject {
    ...
    /// DepthBufferのみのFramebufferを作成する。
    pub fn new_depth_only(
        device: Rc<Device>,
...
impl PipelineObject {
    ...
    /// GraphicsPipelineObjectを生成する。
    /// Vertexなしの不透明の描画用パイプライン。
    pub fn new_opaque_no_vertex(
        device: Rc<Device>,
        vertex_shader_pass: &Path,
        fragment_shader_pass: &Path,
        width: u32,
